...
| 4 | ARGUMEND | argumend.org | NOT-A-COUNTEREXAMPLE (chair correction, pass 7) | Chair re-fetch 2026-08-13 (home/analyze/about pages, browser UA — site 403s generic fetchers): the curated maps (156 topics) carry an explicit root-level verdict axis — "Balance shows which way the evidence tips" (e.g., "Leans toward the counterclaim — moderately evidenced") — so ARGUMEND fails H3's transparency-only term (same exclusion axis as Loki). The "Analyze" mode DOES accept user-pasted text (up to 50k chars), correcting the battery's "pre-selected topics only" description — but it performs local argument-structure extraction of the submitted document ("Source text isn't stored or sent to an AI model"), i.e., document argument-mining, not claim decomposition with external evidence retrieval. Evidence links are narrative ("Every claim links to its evidence"), no study-design/retraction/N metadata. Nearest automated neighbor; its "Crux Identification" principle ("the specific evidence that would change minds") is shipped prior art for the disagreement-localization framing — citation obligation on any localization contribution claim, and strong demand evidence. |
...
**H3' Verdict (as amended by chair, pass 7):** No counterexample found. One qualifier is forced, not two: "automated" — Society Library and Kialo do decomposed symmetric no-verdict inspection but only via human curation. The battery's second qualifier ("user-submitted composite claim") is WITHDRAWN: it was forced only by ARGUMEND, whose corrected characterization (root Balance/Weight verdicts) excludes it on the existing transparency-only term instead. **H3″ ADOPTED: "No automated system offers symmetric transparency-only inspection at the decomposed-premise level of a composite claim."** Fewer conjuncts survive scrutiny better (red-team 1a's gerrymandering warning applies to qualifier accretion). All near-misses must be named in downstream copy: Consensus Meter and ARGUMEND (excluded on transparency-only — both render root-level aggregates), Society Library and Kialo (excluded on automated).
