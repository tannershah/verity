...
**Pass 6/7 addendum (2026-08-13).** Committee-run existence battery (STRESS_SEARCH.md §F) plus chair verification: no counterexample, one qualifier forced. **H3″ ADOPTED: "No automated system offers symmetric transparency-only inspection at the decomposed-premise level of a composite claim."** Near-misses, to be named in all downstream copy: Consensus Meter (product-025) and ARGUMEND (product-027) are excluded on the transparency-only term — both render root-level aggregates (stance vote-count; Balance/Weight evidence-direction verdicts); The Society Library (product-026) and Kialo (product-019) are excluded on the new automated qualifier — decomposed symmetric no-verdict inspection, human-curated. The battery's proposed second qualifier ("user-submitted composite claim") was withdrawn after chair re-verification of ARGUMEND (its Analyze mode is local document argument-mining, not claim decomposition with evidence retrieval; it is excluded on transparency-only regardless) — qualifier accretion is the red team's 1a failure mode and is to be resisted. ARGUMEND's "Crux Identification" principle is shipped prior art for the disagreement-localization framing: citation obligation on any localization claim, and demand evidence for the design. All downstream copy must use H3″.
...
**Pass 6/7 addendum (2026-08-13).** Committee-run battery (STRESS_SEARCH.md §F): no productized counterexample; H5′ holds, restated with the research-line concession carried affirmatively: the reflection/inspection paradigm already exists in research — Meyer et al. 2024 (lit-032, independent conceptual replication in the reflection direction, N=2,036, zero author overlap; the closest empirical support on record for Verity's mechanism, with a high-predisposition null as the honest audience boundary) and Belief Explorer (CHI 2026, lit-034, prototype only, WATCH ITEM) — while deployed products remain persuasion-shaped (debunkbot.com). If Belief Explorer or the Meyer paradigm commercializes, H5′ dies; monitor.
...
**Pass 7 addendum (2026-08-13).** The near-miss landscape is now mapped and named: Consensus (product-025: atomic claim + study-level metadata + root stance aggregate), ARGUMEND (product-027: automated symmetric maps + root Balance/Weight verdicts + crux identification), The Society Library (product-026: premise-level + evidence links, human-curated), Elicit (product-028: production study-metadata extraction, no decomposition). Positioning consequence, committee-adopted: **lead with the conjunction argument and the invalidation moat, not the empty cell** — and cite the near-misses as demand evidence: four independent teams shipped adjacent pieces of this design, which answers "is the cell empty because nobody wants it?" better than any prior line in this record. The moat's evidence now splits explicitly into **impact** — VITALITY (lit-031): excluding retracted RCTs reverses pooled-effect direction in 8.4% and changes statistical significance in 16.0% of contaminated meta-analyses — and **uptake**, where the RetractoBot RCT null (lit-033) convicts post-hoc notification to past citers, and decision-time surfacing to readers, editors, and synthesists remains Verity's stated bet. Copy that conflates impact with uptake is banned (CLAIMS.md).
...
2. **Invalidation propagation.** JTMS layer (Doyle 1979, lit-026) over the alethiology + synthetic retraction-injection tests: mark a KB leaf retracted (as a Retraction Watch/Crossref update would), assert every dependent premise flips OUT and every affected claim graph re-renders flagged. Nearest neighbors for test design: NELL belief demotion (lit-025), Wikidata deprecated-rank handling (report B, Area 6c), and the retracted-papers-on-Wikipedia persistence study (arXiv:2509.18403 — verified 2026-08-13, STRESS_SEARCH.md §E: 71.6% of retracted-paper citations initially problematic, median persistence 3.68 years; also a candidate seed corpus for the linkage eval alongside VITALITY, lit-031).
...
**Problem.** Composite claims travel as take-it-or-leave-it units. A reader confronting "X causes Y" cannot see which premises the claim stands on, how strong the evidence under each premise is, or whether a load-bearing source has been retracted — 66,000+ retractions are machine-readable via Retraction Watch/Crossref (retractionwatch.com, accessed 2026-08-13; ~55,000 as of end-2024, report B, Area 6e), yet nothing downstream of a retracted paper gets flagged (H7). Existing tools either issue verdicts (PolitiFact, product-003; Originality.ai, product-007) — demanding trust transfer that fails exactly on contested claims — or offer transparency only at the article/source/citation level (H3 counterexamples), leaving the claim itself opaque.
...
**Verification debts** (updated pass 7): DISCHARGED — FEVER citation (pass 5, REMEDIATION_LOG J.1); lit-022/lit-026/lit-027 primary URLs (pass 5, J.2–J.4); arXiv:2509.18403 (pass 4.5, STRESS_SEARCH §E). REMAINING — RedacTek row is single-secondary (Doody's review only; vendor primary documentation never obtained); pricing and vendor-accuracy figures are marketing-claims; SEER/NLDR/Task-1 successor figures embargoed pending PDF verification (CLAIMS.md); Dartmouth/Nyhan DebunkBot replications are reported-unpublished; single-pass pass-7 rows flagged verify-before-proposal in their extraction notes (product-026, -028, -029; lit-034).
