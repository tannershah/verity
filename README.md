...
src/verity/presentation/  the terminal render — tree layout, the scorer's own caveat, and the driver that composes the pipeline
...
The render is where the pipeline becomes readable. `python -m verity.presentation render data/demo/spinach.json` draws a committed graph and costs nothing — no key, no checkpoint, no network — and `--json` writes the render payload to stdout with the view on stderr. `python -m verity.presentation run "<claim>"` runs the pipeline for one claim instead: decompose, score every step, bind the identifiers a registry resolves, ground what binds, store, render. That one spends an LLM call, and `--out data/demo/spinach.json` is how the committed graph is re-recorded — the decomposer is not deterministic, so a fresh run replaces it rather than reproducing it.

**What the committed graph does and does not show.** It is one unedited run, stamped with the run id and configuration hash that produced it. One premise in it grounds: the decomposer proposed a DOI, a registry resolved it, and the alethiology holds a verified-primary fact under that exact key — so the row renders `verified` and `live` with the fact's own attributive statement beside it, which is what keeps `verified` from reading as "this premise was checked". That grounding is **not** the pre-registered measurement in [evaluation.md](docs/evaluation.md) §2: the identifier came from the decomposer rather than from retrieval, which is circular, and the run says so in its own notes. Whether any premise grounds at all still varies run to run — two of the eight decompositions run so far proposed an identifier, and the authoritative binding arrives with M6-T3's key attribution. The evidence and retraction columns stay empty until retrieval fills them.
