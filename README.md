...
Both caches live under `.cache/` and are disposable: deleting the directory costs a re-run and can never change an answer, since every key includes a digest of the source tree. `--no-cache` recomputes every stage without deleting anything.

**What the committed graph does and does not show.** It is one unedited run, stamped with the run id and configuration hash that produced it. One premise in it grounds: the decomposer proposed a DOI, a registry resolved it, and the alethiology holds a verified-primary fact under that exact key — so the row renders `verified` and `live` with the fact's own attributive statement beside it, which is what keeps `verified` from reading as "this premise was checked". **How deep the tree goes is a fact about the decomposer, not about the descent.** Recursion runs on premises the decomposer did not type as citation-shaped, so a claim whose premises are all citable in one step produces a shallow tree — on the five pilot decompositions, one premise in thirty-one is a recursion candidate. The run reports its own fan-out and the termination mix beside it, so a flat tree reads as what it is. That grounding is **not** the pre-registered measurement in [evaluation.md](docs/evaluation.md) §2: the identifier came from the decomposer rather than from retrieval, which is circular, and the run says so in its own notes. Whether any premise grounds at all still varies run to run — two of the eight decompositions run so far proposed an identifier, and the authoritative binding arrives with M6-T3's key attribution. The evidence and retraction columns stay empty until retrieval fills them.
