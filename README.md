...
**What the committed graph does and does not show.** It is one unedited run, stamped with the run id and configuration hash that produced it. One premise in it grounds: the decomposer proposed a DOI, a registry resolved it, and the alethiology holds a verified-primary fact under that exact key — so the row renders `verified` and `live` with the fact's own attributive statement beside it, which is what keeps `verified` from reading as "this premise was checked". **How deep the tree goes is a fact about the decomposer, not about the descent.** Recursion runs on the premise types the configured predicate names — `statistical` by default, since a premise typed `empirical-citable` *is* the grounding target and one typed `definitional` or `background` is what no identifier can settle. So a claim whose premises are all citable in one step produces a shallow tree: on the five pilot decompositions, one premise in thirty-one is a recursion candidate. A run reports its own fan-out and termination mix beside the tree, so a flat tree reads as what it is — the committed graph predates the descent and carries neither, which is what Phase 5's re-record makes it. That grounding is **not** the pre-registered measurement in [evaluation.md](docs/evaluation.md) §2: the identifier came from the decomposer rather than from retrieval, which is circular, and the run says so in its own notes. Whether any premise grounds at all still varies run to run: of the decompositions committed here, the five in `data/verifier/pilot/` proposed no identifier at all and this graph proposed one, so the beat depends on the decomposer volunteering a key until M6-T3's attribution binds one from retrieval. The evidence and retraction columns stay empty until retrieval fills them.
